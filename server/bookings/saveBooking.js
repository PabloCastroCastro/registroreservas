...
const save = async (booking, customer) => {
    ...
    const customers = await executeQuery('SELECT customer_id FROM casademiranda.customers WHERE identifier=?', [customer.dni]);
    let idCustomer = 0;
    ...
    const idBooking = await executeQuery('INSERT INTO casademiranda.bookings (check_in, check_out, confirmation_number) VALUES (?, ?, ?);', [booking.checkInDate.split("T")[0], booking.checkOutDate.split("T")[0], booking.numeroConfirmacion]);
    const idBookingCustomer = await executeQuery('INSERT INTO casademiranda.booking_customer (booking_id, customer_id) VALUES (?, ?)', [idBooking.insertId, idCustomer])
...
const saveRoom = async (rooms, idBooking) => {
    ...
    for (var i = 0; i < rooms.length; i++) {
        ...
        const idBookingRoom = await executeQuery('INSERT INTO casademiranda.booking_room (booking_id, room_id, price) VALUES (?, ?, ?)', [idBooking.insertId, idRoom[0].room_id, rooms[i].precio]);
