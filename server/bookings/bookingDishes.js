...
export async function listBookingDishes(bookingId) {
    return executeQuery(
        `SELECT bd.booking_dish_id, bd.dish_id, bd.dish_name, bd.portion_type, bd.price, bd.quantity, bd.dinner_date, bd.created_at
         FROM casademiranda.booking_dishes bd
         WHERE bd.booking_id = ?
         ORDER BY bd.dinner_date, bd.created_at`,
        [bookingId]
    );
}

export async function addBookingDish(bookingId, dishId, portionType, quantity, dinnerDate) {
    ...
    const result = await executeQuery(
        `INSERT INTO casademiranda.booking_dishes (booking_id, dish_id, dish_name, portion_type, price, quantity, dinner_date)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [bookingId, dishId, dish.name, portionType, price, quantity, dinnerDate]
    );
