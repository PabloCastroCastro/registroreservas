...
const update = async (booking_id, customer) => {
    ...
    if (customers.length === 1 && idBooking.length === 1 && customers[0].customer_id === idBooking[0].customer_id) {
        customerUpdate = await executeQuery('UPDATE casademiranda.customers  SET name = ? , surname = ?, identifier = ?, email = ?, nacionality = ?, document_type = ?, support_document = ?, expedition_date = ?, gender = ?, relationship = ?, birthdate = ?, phone = ?, other_phone = ? WHERE customer_id = ?;',
            [customer.nombre, customer.apellidos, customer.numero_documento, customer.email, customer.nacionalidad, customer.tipo_documento, customer.soporte_documento, customer.fecha_expedicion.split("T")[0], customer.genero, customer.parentesco, customer.fecha_nacimiento.split("T")[0], customer.telefono, customer.otro_telefono, customer.cliente_id]);
