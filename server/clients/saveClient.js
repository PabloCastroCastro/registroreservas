...
const save = async (booking_id, customer) => {
    ...
    if (customers.length == 0) {
        const customerInserted = await executeQuery('INSERT INTO casademiranda.customers (name, surname, identifier, email, nacionality, document_type, expedition_date, gender, birthdate,made_booking) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0);',
...
const update = async (booking_id, customer) => {
    ...
    if (customers.length === 1 && idBooking.length === 1 && customers[0].customer_id === idBooking[0].customer_id) {
        customerUpdate = await executeQuery('UPDATE casademiranda.customers  SET name = ? , surname = ?, identifier = ?, email = ?, nacionality = ?, document_type = ?, expedition_date = ?, gender = ?, birthdate = ? WHERE customer_id = ?;',
    ...
    const customerUpdated = await executeQuery('SELECT c.customer_id, name, surname, identifier, email, nacionality, document_type, expedition_date, gender, birthdate, made_booking, b.check_in FROM casademiranda.customers c INNER JOIN casademiranda.booking_customer bc ON c.customer_id = bc.customer_id INNER JOIN casademiranda.bookings b ON bc.booking_id = b.booking_id WHERE c.customer_id=?;', [customer.cliente_id]);
